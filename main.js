...
try{
		...
		if (update.connection == "open" || update.receivedPendingNotifications == "true") {
            ...
            console.log(color(`${themeemoji} GITHUB: B4mbang-94nt3ng `,'magenta'))
...
XeonBotInc.ev.on("messages.upsert",  () => { })
    ...
    let XeonBlokMsg = await XeonBotInc.sendTextWithMentions(XeonFucks.from, `*${XeonBotInc.user.name}* Tidak bisa menerima panggilan ${XeonFucks.isVideo ? `video` : `voice` }. Maaf @${XeonFucks.from.split('@')[0]} anda telah diblokir. Jika ingin membuka silahkan hubungi owner`)
...
"sourceUrl": `${wagc}`}}})
                    ...
                    xeonbody = `Selamat Tinggal @${xeonName.split("@")[0]}\nJangan Lupa Baliknya Bawa Gorengan yaa Mwehehe`
XeonBotInc.sendMessage(anu.id,
 { text: xeonbody,
 contextInfo:{
 mentionedJid:[num],
...
const xeondate = moment.tz('Asia/Kolkata').format('DD/MM/YYYY')
let xeonName = num
xeonbody = ` Selamat yaa 🎉 @${xeonName.split("@")[0]}, anda telah di promosikan sebagai *admin* 🥳`
   XeonBotInc.sendMessage(anu.id,
 { text: xeonbody,
 contextInfo:{
 mentionedJid:[num],
...
const xeondate = moment.tz('Asia/Kolkata').format('DD/MM/YYYY')
let xeonName = num
xeonbody = `Maaf ‼️ @${xeonName.split("@")[0]}, Anda telah dicopot jabatannya dari *admin* ke *member biasa* 😬`
XeonBotInc.sendMessage(anu.id,
 { text: xeonbody,
 contextInfo:{
 mentionedJid:[num],
